// Create client/src/pages/Register.tsx dynamically following layout matching login structures:
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-2xl shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-500">Begin journey toward zero net footprint tracking</p>
        </div>
        {error && <div className="p-3 text-xs bg-red-50 text-red-600 rounded-lg font-medium">{error}</div>}
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider mb-1 text-slate-500">Full Name</label>
            <input type="text" required className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm focus:outline-emerald-500" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
        ...
        <p className="text-center text-sm text-slate-500">Already verified? <Link to="/login" className="text-emerald-600 font-semibold hover:underline">Sign In</Link></p>
      </div>
    </div>
  );
};
